/*
 ...
 필수!!!!!!! 반드시 제어 캔을 연결 한 후, 샤시 캔을 연결해야 함.
 (제어캔, 샤시캔 구분하는 방법 찾아서 캔 연결 순서를 무관하게 만들어보기)
 
 */

//#define CAN_CONTROL_STATUS_PRINT
...
bool print_vehicle_speed_flag = 1;
...
bool print_new_msg_engine_rpm_flag = 0;
bool print_rpm_without_electro_flag = 0;
bool print_brake_pressure_flag = 0;
bool print_whl_spd_flag = 1;
bool print_whl_pul_flag = 0;

// state
u_int16_t APM_state = APM_En;   // APM_D_En, APM_En
u_int16_t ASM_state = ASM_En;   // ASM_D_En, ASM_En
...
int main()
{
    ...
    while(1)
    {
        ...
        //        if(can_chassis.frame.can_id == 0x162) TCU_DVT13_Driving_Tq(can_chassis);
        //        if(can_chassis.frame.can_id == 0x162) TCU_DCT13_Engine_RPM(can_chassis);
...
#ifdef CAN_CONTROL_STATUS_PRINT
